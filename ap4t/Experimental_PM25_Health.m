...
% Log-linear function
Spatial_Mortality_Adult = sum((MR_Matrix.*(One_Mat'.*(1-(1./(exp(DR_Matrix'.*PM25_Exp')))))').*Pop_Adult,2);
...
% Logistic function
Spatial_Mortality_Infant = sum((MR_Matrix.*(One_Mat'.*(1-(1./((One_Mat-MR_Matrix)'.*exp(DR_Matrix'.*PM25_Exp')+MR_Matrix'))))').*Pop_Infant,2);
...
Exp_Spatial_Mortality = Spatial_Mortality_Adult + Spatial_Mortality_Infant;
Exp_Spatial_Damage = Spatial_Damage_Adult + Spatial_Damage_Infant;
Exp_Damage = (Damage_Adult + Damage_Infant);
...
if age == 1
    % Adult, all-cause mortality
    % Log-linear function
    Spatial_Mortality_Adult = (MR_Matrix.*(One_Mat'.*(1-(1./(exp(DR_Matrix'.*PM25_Exp')))))').*Pop_Adult;
    Spatial_Damage_Adult = wtp.*Spatial_Mortality_Adult;
    Damage_Adult = sum(wtp.*Spatial_Mortality_Adult);

    % Infant, all-cause mortality
    % Logistic function
    Spatial_Mortality_Infant = (MR_Matrix.*(One_Mat'.*(1-(1./((One_Mat-MR_Matrix)'.*exp(DR_Matrix'.*PM25_Exp')+MR_Matrix'))))').*Pop_Infant;
    Spatial_Damage_Infant = wtp.*Spatial_Mortality_Infant;
