...
% Log-linear function
Adult_Deaths = sum((MR_Data.*(One'*(1-(1./(exp(dr.*Delta_PM_25')))))').*Pop_Exposed,2);
Adult_Spatial_Damage = wtp.*Adult_Deaths;
Adult_Mortality_Damage = sum(sum(wtp.*Adult_Deaths));

%% Infant, all-cause mortality
% Logistic function
Infant_Deaths = sum((MR_Data.*(One_Mat'.*(1-(1./((One_Mat-MR_Data)'.*exp(dr_infant.*Delta_PM_25')+MR_Data'))))').*Pop_Under1,2);
